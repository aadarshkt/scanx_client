...
const locations = new Set(["Library", "SAC"]);

export default HomeScreen = ({ route, navigation }) => {
  //get token for authorization header from global store.
  const createRecord = async (data, profile) => {
    try {
      console.log("createRecord request " + data);
      await axios
        .post(`http://${baseURL}/location/createRecord`, profile, { params: { location: data } })
        .finally(() => console.log("Record Created"));
    } catch (error) {
      console.error("Error creating Record", error);
    }
  };

  const [home_screen_data, set_home_screen_data] = useState({
    last_location: "",
  ...
  console.log("First route params is" + route.params?.data);
  console.log("whether from camera screen " + route.params?.is_from_camera_screen);
  ...
  useFocusEffect(
    useCallback(async () => {
      console.log("route params is" + route.params?.data);
      const create_record = async () => {
        if (
          route.params?.data !== undefined &&
          locations.has(route.params?.data) &&
          route.params?.is_from_camera_screen === true
        ) {
          const profile = await getProfile();
          await createRecord(route.params.data, profile);
        }
      };
      await create_record();
      console.log("create record useEffect run");
    }, [])
  );
  ...
  useFocusEffect(
    useCallback(() => {
      const fetch_home_screen_data = async () => {
        const get_home_screen_data = async () => {
          try {
            const home_screen_value = await axios.get(`http://${baseURL}/students/last_location`, {
              headers: {
        ...
        if (token != null) {
          const home_screen_data = await get_home_screen_data();
          console.log(home_screen_data);
          set_home_screen_data({
            last_location: home_screen_data.last_location,
            total_sac_time_spent: format_time(home_screen_data.total_sac_time_spent),
            total_library_time_spent: format_time(home_screen_data.total_library_time_spent),
          });
        }
      };
      fetch_home_screen_data();
      console.log("get_home_screen_data useEffect run");
    }, [])
  );

  return (
    <View style={styles.container}>
      <HomeCard label="Last Location" data={home_screen_data.last_location} />
      <HomeCard label="Total SAC time spent" data={home_screen_data.total_sac_time_spent} />
      <HomeCard label="Total Library time spent" data={home_screen_data.total_library_time_spent} />
      <View style={{ marginTop: 20 }}>
        <Button title="go to camera" onPress={() => navigation.navigate("Camera", { parent: "Home" })}>
          Camera
        </Button>
      </View>
    </View>
